...
| `exploring` | 요구사항 탐색 중 (PO와 대화) | `exploration.started` |
...
| `constraint.evidence_updated` | Constraint의 `evidence_status`를 사후 변경. PO가 정책 문서 확인 후 `verified`로 갱신 |
| `prd.rendered` | PRD(Product Requirements Document) 생성 완료. compile.completed 직후 자동 생성 |
| `pre_apply.review_completed` | Pre-Apply Review 완료. verdict(pass/gap_found) + findings |

### Exploration 이벤트

Exploration 이벤트는 **전이 이벤트**입니다. `exploring` 상태의 MATRIX에 정의되어 있습니다.

| Type | Trigger | Payload |
|------|---------|---------|
| `exploration.started` | Exploration 시작 (grounded → exploring 전이) | `{ entry_mode, initial_goals }` |
| `exploration.round_completed` | Round 완료 (exploring 자기전이) | `{ phase, phase_name, round, topic, decisions, assumptions_found?, alternatives_presented?, brief_fields_contributed? }` |
| `exploration.phase_transitioned` | Phase 전환 (exploring 자기전이) | `{ from_phase, to_phase, reason, backward?, log_path, log_hash }`. log_path와 log_hash는 필수 |

Gate-guard 규칙:
- Rule 7: `MAX_EXPLORATION_ROUNDS`(20) 초과 시 round_completed 거부
- Rule 8b: `exploration_progress`가 없으면 round_completed/phase_transitioned 거부
- Rule 8c: exploration 진행 중이면 exploration.started 중복 발행 거부
...
관찰 이벤트 (`feedback.classified`, `convergence.*`, `draft_packet.rendered`, `constraint.evidence_updated`)는 Event Classification 섹션에 정의되어 있습니다.
...
- `draft_packet.rendered`: `{ packet_path, packet_hash, surface_hash, constraint_count, required_count, recommended_count, invalidated_count }`
- `constraint.evidence_updated`: `{ constraint_id, evidence_status, evidence_note?, requires_policy_change? }`. evidence_status는 `verified`, `code_inferred`, `brief_claimed`, `unverified` 중 하나
- `prd.rendered`: `{ prd_path, prd_hash, build_spec_hash, section_count }`. compile.completed 직후 에이전트가 자동 생성

### Constraint — Owner: `kernel/`

타입명과 payload는 여기서 확정합니다. 상세 lifecycle은 `dev-docs/spec/constraint-discovery.md` 참조.

| Type | Trigger | Payload |
|------|---------|---------|
| `constraint.discovered` | 시스템이 constraint 발견 | `{ constraint_id, perspective, summary, severity, discovery_stage, decision_owner, impact_if_ignored, source_refs, evidence_status?, evidence_note?, requires_policy_change? }` |
...
| `compile.completed` | Build Spec 생성 완료 | `{ build_spec_path, build_spec_hash, brownfield_detail_path, brownfield_detail_hash, delta_set_path, delta_set_hash, validation_plan_path, validation_plan_hash }` |
...
| `validation.completed` | 검증 완료 | `{ result, pass_count, fail_count, items }` |
...
| `constraint.discovered` | ⟳ `grounded` (Grounding 단계 constraint 발견) |
...
| Event | 결과 |
|-------|------|
| `exploration.round_completed` | ⟳ `exploring` |
| `exploration.phase_transitioned` | ⟳ `exploring` |
| `align.proposed` | → `align_proposed` |
| `constraint.discovered` | ⟳ `exploring` |
| `snapshot.marked_stale` | ⟳ `exploring` |
| `redirect.to_grounding` | ← `grounded` (소스 근본 변경 시 Grounding 재실행) |
...
| Event | 결과 |
|-------|------|
| `constraint.discovered` | ⟳ `target_locked` |
...
| Event | 결과 |
|-------|------|
| `constraint.discovered` | ⟳ `compiled` |
| `compile.constraint_gap_found` | → `constraints_resolved` (Pre-Apply Review gap) |
...
| constraint evidence 상태 | 각 `constraint_id`의 `evidence_status`. `constraint.discovered`에서 초기화(기본값 `unverified`), `constraint.evidence_updated`로 갱신 |
...
| 시점 | 범위 | 동작 |
|------|------|------|
| Gate 전이 (필수) | 로컬 소스 자동 (`add-dir`, `obsidian-vault`) + 원격 소스는 에이전트가 재스캔 판단 | `source_hashes`를 현재 소스와 비교. 불일치 시 `snapshot.marked_stale` 이벤트 발행 |
| 명령 시작 (경량) | 로컬 소스만 (`add-dir`, `obsidian-vault`) | 해시 비교. 불일치 시 경고 표시. 전이 차단하지 않음 |

Stale 감지는 commands/ 레이어(`stale-check.ts`)에서 수행됩니다. reducer는 `snapshot.marked_stale` 이벤트만 봅니다. 원격 소스(`github-tarball`, `figma-mcp`)의 stale 감지는 네트워크/MCP 접근이 필요하므로 에이전트가 판단합니다.

### source_hashes 키 규약

`source_hashes: Record<string, string>`의 키는 `{type}:{identifier}` 형식을 사용합니다.

| 소스 유형 | 키 형식 | 예시 |
|-----------|---------|------|
| `add-dir` | `add-dir:{path}` | `add-dir:/projects/app/src` |
| `github-tarball` | `github-tarball:{url}` | `github-tarball:https://github.com/org/repo` |
| `figma-mcp` | `figma-mcp:{file_key}` | `figma-mcp:xxxxx` |
| `obsidian-vault` | `obsidian-vault:{path}` | `obsidian-vault:/vaults/docs` |

### 해시 비교 대상

| 소스 유형 | 해시 방식 | 한계 |
|-----------|----------|------|
| `add-dir` (로컬) | 디렉토리: 파일 경로 UTF-16 코드유닛 순서 정렬 (JavaScript 기본 `.sort()`) → 각 파일의 contentHash → sorted hash list concat → 전체 contentHash. 단일 파일: contentHash | — |
| `github-tarball` | tarball의 content hash | — |
| `figma-mcp` | Figma file의 `lastModified` timestamp | content hash가 아닌 timestamp. false positive(메타데이터 변경) 가능 |
| `obsidian-vault` | add-dir과 동일 방식. `.obsidian/` 디렉토리 제외 | — |

Stale 감지 시 source_hashes의 소스 단위 해시가 변경되면, scanner가 파일 단위 비교를 수행하여 `stale_sources`에 개별 파일 경로를 기록합니다.
...
| `build/brownfield-detail.md` | `compile.completed` | `brownfield_detail_path` + `brownfield_detail_hash` |
| `build/delta-set.json` | `compile.completed` | `delta_set_path` + `delta_set_hash` |
| `build/validation-plan.md` | `compile.completed` | `validation_plan_path` + `validation_plan_hash` |
| `build/exploration-log.md` | `exploration.phase_transitioned` | `log_path` + `log_hash` |
| `build/prd.md` | `prd.rendered` | `prd_path` + `prd_hash` |
