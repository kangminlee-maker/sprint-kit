...
개요 및 결정 종류(inject/defer/override/clarify/modify-direction)는 `dev-docs/spec/architecture.md`의 Constraint Discovery 섹션을 참조하세요.
...
Compile Defense(체크리스트 + 감사 통과)가 누락과 왜곡을 잡습니다 (`dev-docs/spec/architecture.md` 참조).
...
scope를 `constraints_resolved`로 되돌립니다 (`dev-docs/spec/event-state-contract.md` 참조).
...
`decision_owner`는 `constraint.discovered` 시점에 시스템이 제안하고, `constraint.decision_recorded` 또는 `constraint.clarify_resolved` 시점에 사용자가 확정합니다. 사용자가 동의하지 않으면 변경할 수 있습니다. `status`가 `undecided`인 동안의 `decision_owner`는 시스템 제안값이며, `decided`로 전이된 후에는 확정값입니다.
...
`dev-docs/spec/event-state-contract.md`에 정의된 constraint 이벤트의 payload를 확정합니다.
...
```json
{
  ...
  "discovery_stage": "grounding | draft_surface_gen | draft_phase1 | draft_phase2 | compile | apply",
  ...
  "evidence_status": "verified | code_inferred | brief_claimed | unverified",
  "evidence_note": "정책 문서 참조 또는 미확인 사유",
  ...
  "constraints": [
    {
      ...
      "decision_owner": "product_owner",
      "impact_if_ignored": "차단 버튼을 눌러도 실제 매칭에 반영되지 않습니다",
      "source_refs": [{ "source": "matching-engine.ts", "detail": "findAvailableTutors()에 exclusion 파라미터 없음" }],
...
`summary.undecided`는 `status: "undecided"` 항목의 수입니다.
