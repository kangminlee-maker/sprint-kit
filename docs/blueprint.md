...
6. **범위 확정** — 포함/제외를 확정합니다. 이것이 Align Packet의 입력이 됩니다
...
- 이벤트: `exploration.started`, `exploration.round_completed`, `exploration.phase_transitioned` (모두 관찰 이벤트)
- [GC-020] Exploration은 `grounded` 상태 내에서 진행됩니다. 새 상태를 추가하지 않습니다
...
- 세션이 중단되면 `exploration-log.md`에서 마지막 Phase와 합의 내용을 읽어 재개합니다
