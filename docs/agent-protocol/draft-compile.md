...
const result = compile({
  ...
  brownfieldDetail,  // 에이전트가 기존 코드 불변 제약 스캔 결과
...
// 2. compile.started 기록 (gate-guard가 retry 상한 검사)
appendScopeEvent(paths, {
...
writeFileSync(join(paths.build, "brownfield-detail.md"), result.brownfieldDetailMd);
...
appendScopeEvent(paths, {
  ...
  payload: {
    ...
    brownfield_detail_hash: result.brownfieldDetailHash,
  },
});
```
...
Pre-Apply Review 발견 사항의 분류 축은 **"새 constraint 생성 필요 여부"**입니다:

- **gap_found**: 기존 constraint pool에 없는 독립적 제약을 발견한 경우. `constraint.discovered` 이벤트 기록이 필요하므로 역전이합니다.
- **warning**: 기존 constraint 범위 내에서 리스크가 식별되었으나, 새 constraint 생성이 불필요한 경우. `finding.status: "warning"`으로 기록하고 진행합니다.
...
// 새 constraint 발견 시 → gap_found verdict 기록 후 역전이
appendScopeEvent(paths, {
  type: "pre_apply.review_completed",
  actor: "agent",
  payload: {
    verdict: "gap_found",
    constraint_gap_id: "CST-NEW",
    findings: [
      { perspective: "policy", status: "pass", summary: "..." },
      { perspective: "brownfield", status: "gap_found", summary: "..." },
      { perspective: "logic", status: "pass", summary: "..." },
    ],
  },
});
appendScopeEvent(paths, {
  type: "constraint.discovered",
  actor: "agent",
  payload: { constraint_id: "CST-NEW", perspective: "brownfield", summary: "...", ... },
});
appendScopeEvent(paths, {
  type: "compile.constraint_gap_found",
  actor: "system",
  payload: { new_constraint_id: "CST-NEW", perspective: "brownfield", summary: "..." },
});
