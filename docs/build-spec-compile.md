...
대응 이벤트: `compile.completed` — `{ build_spec_path, build_spec_hash, delta_set_path, delta_set_hash, validation_plan_path, validation_plan_hash }`
...
현재 시스템에 대해 시스템이 스캔한 모든 정보를 제공합니다. **숨기는 것 없이 전부 포함.**

| 항목 | 필수 | 내용 |
|------|------|------|
| **관련 파일 목록** | 필수 | 변경 대상 및 영향받는 파일. 경로 + 역할 설명 |
| **모듈 의존성** | 필수 | 변경 대상 모듈이 의존하는 모듈 목록 |
| **API 계약** | 선택 | 관련 API endpoint, 요청/응답 형식 |
| **DB 스키마** | 선택 | 관련 테이블 구조 |
| **설정/환경** | 선택 | 관련 환경 변수, 설정 파일 |
...
| 검증 항목 | 내용 |
|-----------|------|
| **inject 반영 확인** | inject 결정인 CST가 Section 4 구현 계획에 포함되어 있는지 |
| **defer 비간섭 확인** | defer 결정인 CST의 `source_refs`에 명시된 파일이 구현 계획에서 해당 constraint와 관련된 방식으로 변경되지 않았는지 |
| **override 비반영 확인** | override 결정인 CST의 `source_refs`에 명시된 파일에서 해당 constraint가 구현에 반영되지 않았는지 |
| **추적 체인 확인** | CST → IMPL → CHG → VAL 체인이 모든 inject 결정에 대해 완전한지 |
...
| `compile.completed` | compile 성공 | Build Spec, delta-set, validation-plan 생성 완료 |
