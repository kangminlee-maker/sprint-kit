...
| `src/compilers/compile-defense.ts` | Layer 1 (Checklist): 모든 constraint가 Build Spec에 참조되는지. Layer 2 (Audit Pass): inject 반영, defer 비간섭(source_refs 범위), override 비반영, 추적 체인(CST→IMPL→CHG→VAL) 완전성 |
...
| 4 | `compile-defense.ts` | checklist 누락 + audit pass + 추적 체인 |
