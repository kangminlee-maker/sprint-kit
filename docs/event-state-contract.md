...
1. 모든 `constraint.discovered`된 constraint에 대해 `constraint.decision_recorded`가 존재해야 합니다
...
타입명은 여기서 확정합니다. payload 상세는 `docs/constraint-discovery.md`(Step 3)에서 정의합니다.

| Type | Trigger | Payload (Step 3에서 확정) |
|------|---------|--------------------------|
| `constraint.discovered` | 시스템이 constraint 발견 | `{ constraint_id, perspective, summary, ... }` |
| `constraint.decision_recorded` | 사용자 또는 Builder가 결정 (inject/defer/override) | `{ constraint_id, decision, selected_option, decision_owner: "product_owner" or "builder", ... }` |
| `constraint.clarify_requested` | 사용자가 clarify 선택 | `{ constraint_id, reason, ... }` |
| `constraint.clarify_resolved` | clarify 해소 후 결정 | `{ constraint_id, resolution, decision, ... }` |
...
| constraint 결정 상태 | 각 `constraint_id`의 가장 최근 `constraint.decision_recorded` 또는 `constraint.clarify_resolved` |
| clarify 상태 | `constraint.clarify_requested` 후 `constraint.clarify_resolved`가 없는 항목 |
...
| `state/constraint-pool.json` | `constraint.discovered`, `constraint.decision_recorded`, `constraint.clarify_requested`, `constraint.clarify_resolved` |
