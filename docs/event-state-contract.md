...
- `constraint.evidence_updated`: `{ constraint_id, evidence_status, evidence_note? }`. evidence_status는 `verified`, `code_inferred`, `brief_claimed`, `unverified` 중 하나
...
| Type | Trigger | Payload |
|------|---------|---------|
| `constraint.discovered` | 시스템이 constraint 발견 | `{ constraint_id, perspective, summary, severity, discovery_stage, decision_owner, impact_if_ignored, source_refs, evidence_status?, evidence_note? }` |
