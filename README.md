...
온톨로지 bundle을 명시하려면 `add-dir` 또는 `github-tarball` source에 `content_role: ontology_bundle`를 지정합니다.
...
`ontology_files`는 nested path override입니다. 생략하면 source 내부에서 exact filename(`code-mapping.yaml`, `behavior.yaml`, `model.yaml`)을 탐색합니다.
...
권장 계약:
- `content_role: ontology_bundle`로 ontology source를 명시합니다.
- 필요하면 `ontology_files`로 YAML 상대 경로를 지정합니다.
- `github-tarball`은 재현성을 위해 `ref` 지정이 권장됩니다.
- explicit declaration이 없으면 ontology bundle 준비를 수행하지 않습니다.
