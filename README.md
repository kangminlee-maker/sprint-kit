...
| 단계 | 하는 일 | 누가 결정하나 |
|------|---------|-------------|
| **Brief** | 변경 목적, 대상 사용자, 기대 결과를 작성합니다 | PO가 작성 |
| **Align** | 소스를 스캔하고, 현재 상태(as-is)와 요청(to-be) 사이의 충돌을 발견합니다. Align Packet으로 정리하여 PO에게 제시합니다 | PO가 방향 승인 |
...
| 명령어 | 시점 | 하는 일 |
|--------|------|---------|
| `/start` | 시작할 때 | Brief를 작성하고, 소스를 스캔합니다. 결과로 Align Packet이 생성됩니다 |
...
| **L3** | 미검증 가정, 정책 변경 필요 사항 등을 경고 | 경고만 (진행 가능) |
...
| `docs/agent-protocol/` | 에이전트 실행 프로토콜 (start, align, draft) |

## 현재 규모

- 소스 파일 45+개, 테스트 44파일 947건
